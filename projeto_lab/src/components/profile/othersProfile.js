...
class othersProfile extends Component {
    ...
    getPokemonForProfileIQ = (userCorrectAnswers, userWrongAnswers) => {
        ...
        if (isNaN(averageCorrectAnswers)) {
            this.setState({ pokemonIQ: null, isLoading: false })
        ...
        if (this.state.pokemonIQ) {
            ...
            fetch(url)
                ...
                .then((data) => this.setState({ pokemonIQ: data, isLoading: false }))
    ...
    render() {
        ...
        const { loggedUserFriends, pokemonIQ, favoritesResults, teamResults } = this.props;
        const { getPokemonIQ, loading, error } = this.state;
